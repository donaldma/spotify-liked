...
export const createDescription = (start, end) => {
    return `Generated by Spotify Liked (https://spotify-liked.netlify.app) [${start} - ${end}]`
}

// descriptions written by current and older versions of the app
const descriptionPatterns = [
    createDescription('*', '*'),
    'Generated by https://github.com/donaldma/spotify-liked [* - *]',
]

export const isGeneratedDescription = (text) => {
    return descriptionPatterns.some((pattern) => isMatch(text, pattern))
}

// retries rate-limited (429, after Spotify's Retry-After delay) and
// transient server error (5xx) requests
export const withRetry = async (request, retries = 3) => {
    try {
        return await request()
    } catch (error) {
        const rateLimited = error.statusCode === 429
        const transient = [500, 502, 503].includes(error.statusCode)

        if ((!rateLimited && !transient) || retries === 0) {
            throw error
        }
